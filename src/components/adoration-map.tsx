...
export function AdorationMap({ places }: { places: Place[] }) {
  ...
  async function focus(place: Place) {
    ...
    try {
      const response = await fetch(`https://api.mapbox.com/search/geocode/v6/forward?q=${encodeURIComponent(place.query)}&limit=1&access_token=${token}`);
      const payload = await response.json() as { features?: { geometry?: { coordinates?: [number, number] } }[] };
      const coordinates = payload.features?.[0]?.geometry?.coordinates;
      if (coordinates) mapRef.current?.flyTo({ center: coordinates, zoom: 16, pitch: 65, bearing: -22, essential: true });
    } catch { /* La posición de ciudad sigue disponible como respaldo. */ }
  }
