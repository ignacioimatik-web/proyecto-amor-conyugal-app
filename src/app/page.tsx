...
import { GospelPlayer } from "@/components/gospel-player";
...
type OfficialPost = { title: string; date: string; excerpt: string; content: string; contentHtml: string; link: string; audio: string | null };
      ...
      <div className="mt-10 grid items-start gap-10 lg:grid-cols-[.88fr_1.12fr] lg:gap-16">
        ...
        <article className="overflow-hidden rounded-[2.3rem] border border-[#dce7ed] bg-[#fffdf9] shadow-xl shadow-[#4f7087]/10"><div className="bg-[#4f7087] px-7 py-8 text-[#fffaf2] sm:px-10"><p className="text-xs font-bold uppercase tracking-[.2em] text-[#dce7ed]">Evangelio del día</p><h2 className="mt-3 font-serif text-3xl capitalize sm:text-4xl">{displayDate(date)}</h2>{reading && <p className="mt-4 text-sm text-[#e5eff3]">{reading.celebration} · <b>{reading.gospel.reference}</b></p>}</div>
          <div className="px-7 py-8 sm:px-10 sm:py-10">
            ...
            {!loading && reading && <><p className="font-serif text-2xl text-[#243d32]">Lectura del santo Evangelio según san {reading.gospel.book}</p><div className="mt-7 space-y-5 text-[17px] leading-8 text-[#45534b]">{reading.gospel.verses.map((verse) => <p key={verse.number}><sup className="mr-2 text-xs font-bold text-[#d96c4a]">{verse.number}</sup>{verse.text}</p>)}</div><div className="mt-10 space-y-3 border-t border-[#ece5dc] pt-6"><GospelPlayer text={`Lectura del santo Evangelio según san ${reading.gospel.book}. ${reading.gospel.verses.map((verse) => verse.text).join(" ")}`} /><div className="flex flex-wrap gap-3"><button onClick={() => setSaved(!saved)} className="rounded-full bg-[#203f34] px-5 py-3 text-sm font-bold text-white">{saved ? "✓ Guardado para orar juntos" : "♡ Guardar para esta noche"}</button><a href={ceeUrl(date)} target="_blank" rel="noreferrer" className="rounded-full border border-[#d7cec2] px-5 py-3 text-sm font-bold">Contrastar con la CEE ↗</a></div></div><p className="mt-6 text-xs leading-5 text-[#879087]">{reading.source} La selección litúrgica se sirve como orientación y debe contrastarse con la CEE cuando exista una celebración propia de España.</p></>}
          </div>
        </article>
      </div>
    </section>
...
function RetreatCollection({ title, subtitle, groups, past = false }: { title: string; subtitle: string; groups: RetreatGroup[]; past?: boolean }) {
  return <section className="mt-10 rounded-[2rem] bg-[#4f7087] p-7 text-[#fffaf2] sm:p-8"><p className="text-xs font-bold uppercase tracking-[.15em] text-[#dce7ed]">Retiros y misiones</p><div className="mt-2"><h3 className="font-serif text-3xl">{title}</h3><p className="mt-1 text-sm text-[#e4eef3]">{subtitle}</p></div><div className="mt-7 grid gap-6 md:grid-cols-2">{groups.map((group) => <section key={group.country} className="rounded-3xl bg-white/15 p-4"><div className="mb-4 flex items-center gap-3"><span className="h-px flex-1 bg-white/35" /><p className="text-xs font-bold uppercase tracking-[.18em] text-[#e5f0f4]">{group.country}</p><span className="h-px flex-1 bg-white/35" /></div><div className="space-y-4">{group.items.map((retreat) => <article key={retreat.link} className="overflow-hidden rounded-3xl bg-[#fffaf2] text-[#24342e] shadow-sm"><div className="relative h-40 bg-[#d4c7b9]">{retreat.image ? <img src={retreat.image} alt={retreat.title} className="h-full w-full object-cover" /> : <div className="grid h-full place-items-center text-3xl text-[#4f7087]">♡</div>}<span className="absolute left-3 top-3 rounded-full bg-[#4f7087] px-3 py-1 text-xs font-bold text-white">{displayDate(retreat.date)}</span></div><div className="p-5"><h4 className="font-serif text-xl leading-tight">{retreat.title}</h4><p className="mt-3 line-clamp-3 text-sm leading-6 text-[#69766f]">{retreat.excerpt}</p><a href={retreat.link} target="_blank" rel="noreferrer" className="mt-5 inline-flex rounded-full bg-[#4f7087] px-4 py-2 text-xs font-bold text-white">Ver información oficial ↗</a></div></article>)}</div></section>)}</div></section>;
}

function FeaturedReflection({ post }: { post: OfficialPost }) {
  return <article className="overflow-hidden rounded-[2rem] bg-[#4f7087] p-7 text-[#fffaf2] shadow-xl shadow-[#4f7087]/20 sm:p-9"><div className="grid gap-7 lg:grid-cols-[1.15fr_.85fr] lg:items-end"><div><p className="inline-flex rounded-full bg-[#7894a8] px-3 py-1 text-xs font-bold uppercase tracking-[.14em] text-white">Reflexión de hoy</p><p className="mt-4 text-sm font-semibold text-[#dce7ed] capitalize">{displayDate(post.date)}</p><h3 className="mt-3 max-w-2xl font-serif text-3xl leading-tight sm:text-4xl">{post.title}</h3><p className="mt-5 max-w-2xl text-sm leading-6 text-[#e4eef3]">{post.excerpt}</p><details className="group mt-5 overflow-hidden rounded-2xl bg-[#fffaf2] text-[#24342e]"><summary className="flex cursor-pointer list-none items-center justify-between px-5 py-4 text-sm font-bold"><span>Leer reflexión completa</span><span className="text-lg text-[#7894a8] transition group-open:rotate-45">+</span></summary><div className="border-t border-[#ece5dc] px-5 pb-6 pt-5"><p className="mb-6 border-l-2 border-[#7894a8] pl-4 font-serif text-lg leading-7 text-[#385448]">Una lectura para hacer despacio, juntos y en oración.</p><div className="reflection-reading" dangerouslySetInnerHTML={{ __html: post.contentHtml }} /><a href={post.link} target="_blank" rel="noreferrer" className="mt-6 inline-flex text-sm font-bold text-[#5b778c] underline">Ver publicación oficial ↗</a></div></details></div><div className="rounded-3xl bg-[#fffaf2] p-5 text-[#243d32]"><p className="text-xs font-bold uppercase tracking-[.14em] text-[#557265]">Orad juntos</p>{post.audio ? <><p className="mt-2 font-serif text-xl">Escuchad la reflexión</p><audio controls preload="metadata" className="mt-4 h-10 w-full accent-[#7894a8]"><source src={post.audio} type="audio/mpeg" />Tu navegador no admite audio integrado.</audio></> : <p className="mt-2 text-sm text-[#68736b]">La reflexión está disponible en la fuente oficial.</p>}</div></div></article>;
}
...
function OfficialResource({ resource, kind }: { resource: OfficialResource; kind: "testimonios" | "adoraciones" }) {
  ...
  return <article className={`overflow-hidden rounded-[2rem] ${isTestimony ? "bg-[#d96c4a]" : "bg-[#254b3d]"} text-[#fffaf2]`}><div className="grid h-full sm:grid-cols-[.85fr_1.15fr]">{isTestimony ? <div className="aspect-video bg-black sm:h-full"><iframe className="h-full w-full" src="https://www.youtube-nocookie.com/embed/0Mio4WxlUC4" title="Hagan Lío" allow="accelerometer; autoplay; encrypted-media; picture-in-picture" allowFullScreen /></div> : <div className="grid min-h-48 place-items-center bg-[#1d3d32] p-6 text-center font-serif text-2xl">✦<br />Adoración</div>}<div className="p-7 sm:p-8"><p className="text-xs font-bold uppercase tracking-[.16em] text-white/75">{isTestimony ? "Historias que dan esperanza" : "Un lugar para volver al centro"}</p><h3 className="mt-3 font-serif text-3xl">{resource.title}</h3><p className="mt-4 text-sm leading-6 text-white/85">{isTestimony ? "El testimonio ha sido el medio preferido para el Señor para dar a conocer su obra." : "Adoraciones por los matrimonios en el mundo. En España, comenzando por Madrid y sus parroquias."}</p><a href={isTestimony ? "/testimonios" : "/adoraciones"} className="mt-6 inline-flex rounded-full bg-[#fffaf2] px-4 py-2 text-xs font-bold text-[#254b3d]">{isTestimony ? "Ver testimonios" : "Ver adoraciones"}</a></div></div></article>;
}
